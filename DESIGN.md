...
- **Pressure and pile-up.** When the front of the horde hits an obstruction — a tower footprint, a NET dam, a wall — it doesn't politely queue single-file. It **piles up**: density visibly increases at the obstruction, the mass bulges and thickens the way water backs up against a dam, and agents further back keep arriving and compressing into the same crowded space until pressure finds a way through or around.
...
- **Bow waves at obstructions.** Anything the flow cannot pass cleanly — a tower footprint, a NET dam, the outer wall of a switchback the mass takes too fast — should produce a visible backpressure bulge *upstream* of it: the horde looks **compressed**, not just "slowed down," and it should be obvious at a glance rather than something you squint at. Note what this bullet no longer says: the bow wave is **something the player manufactures, not something the map hands them.** Levels do not author single-file pinches (§4.3), so if you want this read, you build the thing that causes it.
...
- **Player-manufactured obstructions** are the third concentration mechanism, and the only one that isn't geometry: a NET dam or a mid-lane tower footprint (§5.5) compresses the horde *where the player chose to spend*, which is how the game delivers §4.2's bow-wave density without any level authoring a pinch for it.
