...
**3. Levels are authored as splines, not painted masks.** A level is a JSON file defining vessel centerline splines with per-point width, plus spawn portals, objective points, and placement zones. At load, splines rasterize into a tissue mask → signed distance field → flow field. This gives the organic, branching, variable-width vessels §4 calls for while remaining fully authorable in text by an agent with no visual editor. A `tools/preview_level.py` script renders a level JSON to a PNG for quick eyeballing.
...
| **2D — Level pipeline** | `game/level/`, `assets/levels/` | Level JSON schema + loader + validation, spawn portals, objective structure and integrity meter, placement-zone definition, and 2 test levels (one chokepoint, one floodplain) that later content agents use as templates. |
